(() => {
  ...
  let results = [];
  let fallbackNews = [];
  let secondsToNextRefresh = Math.round(cfg.refreshMs / 1000);
  let isLoading = false;
  ...
  function updateCountdownText() {
    ...
    if (!el) return;
    const base = el.dataset.baseStatus || el.textContent;
    el.dataset.baseStatus = base;
    el.textContent = `${base} · اگلی تازہ کاری ${secondsToNextRefresh} سیکنڈ میں`;
  }

  function startCountdown() {
    secondsToNextRefresh = Math.round(cfg.refreshMs / 1000);
    clearInterval(window.__ajkCountdownTimer);
    window.__ajkCountdownTimer = setInterval(() => {
  ...
  async function loadAll() {
    ...
    delete $("#connectionText").dataset.baseStatus;
    try {
